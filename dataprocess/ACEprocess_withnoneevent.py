...
def get_none_events(apf_events=[],R=[],sgm_str='',apf_file=''):
    ...
    for R_element in apf_events:
        start=R_element.text_start
        end=R_element.text_end
        text=R_element.text_unremoved#没有被处理过的，没有去除回车的
        text_set.add(text)
    sgm_tmp=sgm_str
    for text in text_set:
        if text in sgm_tmp:
            sgm_tmp=sgm_tmp.replace(text,'')#去除包含事件的句子
        else:
            refailed+=1
            return
            print("filename："+str(apf_file))
            print("text:"+text)
            raise ValueError("text not in sgm!")
    document=en_nlp(sgm_tmp)
...
def extract_ace_info(apf_file, \
    ...
    for node in event_nodes:
        ...
        for mention_node in mention_nodes:
            ...
            R.append(R_element.toDict())#R中加入包含的事件
    get_none_events(apf_events, R,sgm_str,apf_file)#R中加入不存在事件的句子
            # R.append(R_element.toDict())
    return R
    ...
    for dirname in ACEFileList:
        ...
        for fileItem in fileList:
            ...
            if filename_no_end in trainFileList:
                train+=1
                sgm_str=ace_sgm_deleteXMLtag(filename)#返回sgm文件中去除xml标签的部分
                extract_ace_info(filename,trainEventList,sgm_str)#用apf文件读事件
            if filename_no_end in devFileList:
                dev+=1
                sgm_str =ace_sgm_deleteXMLtag(filename)
                extract_ace_info(filename,devEventList,sgm_str)
            if filename_no_end in testFileList:
                test+=1
                sgm_str =ace_sgm_deleteXMLtag(filename)
                extract_ace_info(filename,testEventList,sgm_str)
